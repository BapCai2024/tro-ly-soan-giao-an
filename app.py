...
# 2. Tự động lấy Key từ Secrets
try:
    if "GOOGLE_API_KEY" in st.secrets:
        api_key = st.secrets["GOOGLE_API_KEY"]
        genai.configure(api_key=api_key)
    else:
        st.error("Chưa tìm thấy Key. Hãy vào Settings > Secrets để thêm GOOGLE_API_KEY.")
        st.stop()
except FileNotFoundError:
    st.error("Chưa cấu hình Secrets. Vui lòng kiểm tra lại cài đặt trên Streamlit.")
    st.stop()
...
# 4. Giao diện tải file (Chỉ viết 1 lần duy nhất ở đây)
uploaded_files = st.file_uploader(
    "Tải lên tài liệu tham khảo (Sách GK, Tài liệu chuyên môn...)", 
    type=['pdf', 'docx'], 
    accept_multiple_files=True,
    key="unique_uploader_key" # Thêm key để tránh lỗi trùng lặp tuyệt đối
)

# 5. Nhập yêu cầu
topic = st.text_area("Nhập tên bài dạy và yêu cầu cụ thể:", 
                     placeholder="Ví dụ: Soạn giáo án môn Tiếng Việt lớp 4, bài 'Cây gạo', dạy trong 2 tiết...")
...
if st.button("🚀 Soạn Giáo Án Ngay"):
    if not uploaded_files or not topic:
        ...
        with st.spinner("Đang đọc tài liệu và suy nghĩ..."):
            ...
            Lưu ý: Ngôn ngữ sư phạm chuẩn mực.
